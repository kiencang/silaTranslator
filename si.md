...
3.  **Thuật ngữ Chuyên ngành (Đặc biệt Quan trọng cho Tài liệu Khoa học):**
    ...
    *   **Các thuật ngữ cụ thể cần lưu ý trong tài liệu này:**
        *   `Importance Performance Analysis (IPA)`: Phân tích Tầm quan trọng - Hiệu suất (IPA)
        *   `Online Motorcycle Taxis`: Xe ôm công nghệ / Xe máy dịch vụ trực tuyến
        *   `Ride-sourcing`: Dịch vụ gọi xe (dựa trên ứng dụng)
        *   `SERVQUAL model`: Mô hình SERVQUAL
        *   `Gross Merchandise Value (GMV)`: Tổng giá trị hàng hóa (GMV)
        *   `Bus Rapid Transit (BRT)`: Xe buýt nhanh (BRT)
        *   `Light Rail Transit (LRT)`: Tàu điện nhẹ (LRT)
        *   `Mass Rapid Transit (MRT)`: Tàu điện ngầm/đô thị (MRT)
        *   `Gojek`, `Grab`, `Lyft`: Giữ nguyên tên thương hiệu.
        *   `Yogyakarta City`: Thành phố Yogyakarta
        *   `Tangibles`, `Reliability`, `Responsiveness`, `Assurance`, `Empathy` (trong mô hình SERVQUAL): Các yếu tố hữu hình, Độ tin cậy, Khả năng đáp ứng, Sự đảm bảo, Sự đồng cảm.
        *   `Quadrant 1, 2, 3, 4`: Góc phần tư 1, 2, 3, 4.
